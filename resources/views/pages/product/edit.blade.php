    ...
    <div>
        <div class="container py-24">
            <div class="flex flex-col gap-14">
                <h1 class="text-5xl border-b border-white border-opacity-25 pb-7 text-black">Продукцияне редакцияләү</h1>
                <form action="{{ route('product.update', $product->id) }}" method="post" class="flex flex-col items-start gap-7" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <input type="text" class="input w-full" name="name" placeholder="Продукциянең исеме" value="{{ old('name') }}">
                    @error('name') <p class="text-red-500">{{ $message }}</p> @enderror
                    <textarea name="description" class="input w-full" placeholder="Продукциянең тасвирламасы">{{ old('description') }}</textarea>
                    @error('description') <p class="text-red-500">{{ $message }}</p> @enderror
                    <input type="number" class="input w-full" name="price" placeholder="Бәя" value="{{ old('price') }}">
